...
var slowestKey = function(releaseTimes, keysPressed) {
  ...
  for (let i = 1; i < releaseTimes.length; i++) {
    ...
    if (keysPressed[i] === 'c') {
      console.log(d, duration, i, keysPressed[i], answer)
      console.log(keysPressed[i].charCodeAt(0), answer.charCodeAt[0])
    }
    if (d > duration) {
      duration = d
      answer = keysPressed[i]
    } else if (d === duration && (keysPressed[i].charCodeAt(0) >= answer.charCodeAt(0))) {
      duration = d
      answer = keysPressed[i]
    } 
  }
