/**
 * 
 * 
  Alice and Bob have candy bars of different sizes: A[i] is the size of the i-th bar of candy that Alice has, and B[j] is the size of the j-th bar of candy that Bob has.

  Since they are friends, they would like to exchange one candy bar each so that after the exchange, they both have the same total amount of candy.  
  (The total amount of candy a person has is the sum of the sizes of candy bars they have.)

  Return an integer array ans where ans[0] is the size of the candy bar that Alice must exchange, and ans[1] is the size of the candy bar that Bob must exchange.

  If there are multiple answers, you may return any one of them.  It is guaranteed an answer exists.

  

  Example 1:

  Input: A = [1,1], B = [2,2] -2
  Output: [1,2]
  Example 2:

  Input: A = [1,2], B = [2,3] -2
  Output: [1,2]
  Example 3:

  Input: A = [2], B = [1,3] -2
  Output: [2,3] 
  Example 4:

  Input: A = [1,2,5], B = [2,4] 2
  Output: [5,4]
  

  Note:

  1 <= A.length <= 10000
  1 <= B.length <= 10000
  1 <= A[i] <= 100000
  1 <= B[i] <= 100000
  It is guaranteed that Alice and Bob have different total amounts of candy.
  It is guaranteed there exists an answer.

 * 
 * 
 * 
 */

 /**
 * @param {number[]} A
 * @param {number[]} B
 * @return {number[]}
 */
var fairCandySwap = function(A, B) {
    let totalA = 0
    let totalB = 0 
    for (let i = 0; i < A.length; i++) {
      totalA+=A[i]
    }
    for (let j = 0; j < B.length; j++) {
      totalB+=B[j]
    }
    let subStract = (totalA - totalB)/2 // 一定是偶数
    // 两个数组差 这个 一个值
    // A 数组里面一定有个值 A[i] - B[j] = subStract
    // 要找到这个值，我想到的是暴力法，遍历 m*n次

    // 还有其他办法
    // 一：
    // 但是经过排序之后，可以遍历m+n次找到这个值
    let i = 0, j = 0;
    while(i < A.length && j < B.length) {
      let diff = A[i] - B[j]
      if (diff === subStract) {
        return [A[i], B[j]]
      } else if (diff < subStract) {
        i++
      } else {
        j++
      }
    }
    // 二：
    // 将所有A的元素放在Set里面。然后遍历B的元素，如果（B[i] + subStract）在A里面，那么返回
    // 时间复杂度也是O(n) 很cool！

};